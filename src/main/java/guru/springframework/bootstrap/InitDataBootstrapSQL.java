...
@Component
@Profile({"dev", "prod"})
public class InitDataBootstrapSQL implements ApplicationListener<ContextRefreshedEvent> {
    private final RecipeRepo recipeRepo;
    private final CategoryRepo categoryRepo;
    private final UnitOfMeasureRepo unitOfMeasureRepo;

    public InitDataBootstrapSQL(RecipeRepo recipeRepo, CategoryRepo categoryRepo, UnitOfMeasureRepo unitOfMeasureRepo) {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        if (categoryRepo.count() < 1L) {
            log.debug("Loading Categories");
            loadCategories();
        }
        if (unitOfMeasureRepo.count() < 1L) {
    ...
    private void loadUnitOfMeasures() {
        ...
        eachUoM.setUnit("Each");
        unitOfMeasureRepo.save(eachUoM);

        UnitOfMeasure tspnUoM = new UnitOfMeasure();
        tspnUoM.setUnit("Tspn");
        unitOfMeasureRepo.save(tspnUoM);

        UnitOfMeasure tbspUoM = new UnitOfMeasure();
        tbspUoM.setUnit("Tbsp");
        unitOfMeasureRepo.save(tbspUoM);

        UnitOfMeasure cupUoM = new UnitOfMeasure();
        cupUoM.setUnit("Cup");
        unitOfMeasureRepo.save(cupUoM);

        UnitOfMeasure dashUoM = new UnitOfMeasure();
        dashUoM.setUnit("Dash");
        unitOfMeasureRepo.save(dashUoM);

        UnitOfMeasure ounceUoM = new UnitOfMeasure();
        ounceUoM.setUnit("Ounce");
        unitOfMeasureRepo.save(ounceUoM);
    }
//
